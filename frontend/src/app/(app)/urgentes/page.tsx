...
export default function UrgentesPage() {
  ...
  async function loadData() {
    ...
    if (a) setAutorizaciones((a as Autorizacion[]).filter(x => x.urgencia === 'urgente'))
    setLoading(false)
  }
  ...
  return (
    <>
      <div className="bg-white px-6 py-4 border-b border-gray-200 shrink-0">
        <h1 className="text-lg font-bold text-gray-900">Casos Urgentes</h1>
        <p className="text-sm text-gray-500">Autorizaciones marcadas como urgentes</p>
      </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200">
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead>
                  <tr>
                    {['Paciente','Aseguradora','Procedimiento','Estado','Fecha','Acciones'].map(h => (
                ...
                <tbody>
                  {autorizaciones.length === 0 && (
                    <tr><td colSpan={6} className="py-10 text-center text-sm text-gray-400">No hay casos urgentes.</td></tr>
                  )}
                  {autorizaciones.map(a => (
                    <tr key={a.id} onClick={() => setSelected(a)} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer">
                      ...
                      <td className="px-3.5 py-3 text-sm text-gray-700">{a.procedimiento_descripcion || '–'}</td>
